...
$(document).ready(function(){
	...
	$(".star a").click( function () {
		$(this).prevAll().find("img").attr("src",temp+"/"+style+"/public/images/icon/star_color.png");
		$(this).prevAll().attr("sel", "red");
		$(this).find("img").attr("src",temp+"/"+style+"/public/images/icon/star_color.png");
		$(this).attr("sel", "red");
		$(this).nextAll().find("img").attr("src",temp+"/"+style+"/public/images/icon/star.png");
	...
	$(".star a").mouseout(function(){
		$(this).parent().find("a img").attr("src",temp+"/"+style+"/public/images/icon/star.png");
		$(this).parent().find("a[sel=red] img").attr("src",temp+"/"+style+"/public/images/icon/star_color.png");
	});
});
...
function loadFunction(){
	 ...
	 * @param {Object} even
	 */
	$('.evaluate_right_imgs').on('click','li span',function(){
		var rate_content=$(this).parents('.rate_content');
		$(this).parent().remove();  //必须写到前面不然等图片没了就无法移除！待后期验证先这样无问题
		...
		$.ajax({
			...
			success:function(data){
				if(data == 1){
					$.msg('评价成功');
					setTimeout(function(){
						location.href = shop_main+"/member/myorderlist"
					},1000);
				}
			}
		})
