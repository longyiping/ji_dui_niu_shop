...
$(function(){
	...
	<div class="member-main">
		<div class="box">
			...
			<div class="tabmenu">
				<input type="button" value="新增收货地址" onclick="location.href='SHOP_MAIN/member/addressinsert'" class="main-btn common-btn" />
			</div>
			<?php if($total_count != ''): ?>
			<div>
					...
					<?php if(is_array($list['data']) || $list['data'] instanceof \think\Collection || $list['data'] instanceof \think\Paginator): $k = 0; $__LIST__ = $list['data'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$new): $mod = ($k % 2 );++$k;?>
						<tr class="order-bd last">
							...
							<td align="center" class="baobei no-border-right order_goods_info">
								<div class="last">
									<a href="SHOP_MAIN/member/updatememberaddress?id=<?php echo $new['id']; ?>" class="main-btn main-btn-large">修改</a> 
									<?php if($new['is_default'] != 1): ?>
									<a href="javascript:void(0);" onclick="address_delete(<?php echo $new['id']; ?>)" class="main-btn main-btn-large">删除</a>
									<?php endif; ?>
								</div>
							</td>
						</tr>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_1888__=1;$__FOR_END_1888__=$page_count+1;for($i=$__FOR_START_1888__;$i < $__FOR_END_1888__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_12131__=1;$__FOR_END_12131__=$page_num+1;for($i=$__FOR_START_12131__;$i < $__FOR_END_12131__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="SHOP_MAIN/<?php echo $path_info; ?>?page=<?php echo $i; ?><?php echo $query_string; ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_14738__=$page_count-$page_num+1;$__FOR_END_14738__=$page_count+1;for($i=$__FOR_START_14738__;$i < $__FOR_END_14738__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="SHOP_MAIN/<?php echo $path_info; ?>?page=<?php echo $i; ?><?php echo $query_string; ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_25373__=$page-($page_num-1)/2;$__FOR_END_25373__=$page+($page_num-1)/2+1;for($i=$__FOR_START_25373__;$i < $__FOR_END_25373__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="SHOP_MAIN/<?php echo $path_info; ?>?page=<?php echo $i; ?><?php echo $query_string; ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
